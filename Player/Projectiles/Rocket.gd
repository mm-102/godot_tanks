...
func _physics_process(delta):
	...
	if is_instance_valid(target):
		...
		$Pivot/Sprite.get_material().set_shader_param("radius", distance)
		$Pivot/Particles2D.process_material.set_shader_param("radius", distance)
		var rot = navigation_agent.get_next_location().angle_to_point(position)
		rotation = lerp_angle(rotation, rot, delta * 8)
	else:
		rotation = linear_velocity.angle()
		anim_time += delta * 7
	$Pivot/Sprite.get_material().set_shader_param("current_time", anim_time)
	$Pivot/Particles2D.process_material.set_shader_param("current_time", anim_time)
